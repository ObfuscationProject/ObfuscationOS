...
static void set_gate(std::uint8_t vec, void (*isr)() noexcept) noexcept
{
    ...
    auto addr = reinterpret_cast<std::uint32_t>(isr);
    g_idt[vec].off_low = static_cast<std::uint16_t>(addr & 0xFFFFu);
    g_idt[vec].sel = kCodeSel;
    g_idt[vec].zero = 0;
    g_idt[vec].flags = kFlags;
    g_idt[vec].off_high = static_cast<std::uint16_t>((addr >> 16) & 0xFFFFu);
}

static void load_idt() noexcept
{
    IdtPtr idtr{static_cast<std::uint16_t>(sizeof(g_idt) - 1), reinterpret_cast<std::uint32_t>(&g_idt[0])};
