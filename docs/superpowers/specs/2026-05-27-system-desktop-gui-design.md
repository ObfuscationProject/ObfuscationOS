...
`system-gui` draws a modestly artistic first-run login surface using the compositor: a dark desktop-compatible palette, ObfuscationOS branding, root as the default user, launcher hints, app availability, and live kernel metrics supplied by the scheduler/topology bindings. It opens after the OS loader runs, not during core kernel boot. Debug shell remains available but does not define the default GUI.

## User Experience

The first screen is the desktop, taskbar, launchers, and one ObfuscationOS Login window. The login window uses OS-facing title and text, while the taskbar launchers continue to open terminal and file manager workflows. The task monitor launcher should open the existing task-manager workflow instead of doing nothing. A `system tui` command exposes the same default-login and app context for text-mode use.

## Testing

Roadmap GUI tests verify that `system-gui` is not loaded during core boot, is loaded from the filesystem package by the module-loader syscall or boot fallback metadata, is registered as a non-built-in module, publishes its service, opens a default login surface, loads app package windows, and renders to the framebuffer. Existing compositor, shell, file manager, and task manager tests remain in place.
